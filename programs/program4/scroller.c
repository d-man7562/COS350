...
#define MAX_BUFFER_SIZE 1000000     // Maximum buffer size for file content
#define SCROLL_RATE_CHANGE 0.2      // Rate to increase/decrease scrolling speed
...
int scrolling_active = 0;       // Flag for continuous scrolling
double scroll_interval = DEFAULT_SCROLL_INTERVAL; // Time between scrolls in seconds
struct termios original_termios; // Store original terminal settings
struct itimerval timer;         // Timer for continuous scrolling
...
int main(int argc, char *argv[]) {
    ...
    // Read file from command line or stdin
    if (argc > 1) {
        read_file(argv[1]);
    } else {
        read_file(NULL); // Read from stdin
    }
    ...
    // Main loop - wait for and process keyboard input
    while (1) {
        if (read(keyboard_fd, &c, 1) > 0) {
            handle_keypress(c);
        }
        usleep(10000); // Short sleep to prevent CPU hogging
    }

    // Cleanup and exit (this won't be reached normally)
    close(keyboard_fd);
    cleanup_and_exit();
    return 0;
}
...
void read_file(const char *filename) {
    ...
    // Count lines and allocate line positions array
    ...
    // Record starting position of each line
    line_positions[0] = 0;
    line_count = 1;
    for (i = 0; i < buffer_size; i++) {
        if (buffer[i] == '\n') {
...
void display_screen() {
    ...
    for (i = 0; i < window_rows - 1 && current_line + i < total_lines; i++) {
        ...
        if (line < total_lines - 1) {
            len = line_positions[line + 1] - pos - 1; // -1 to exclude the newline
        } else {
            len = buffer_size - pos;
        }
        
        // Handle long lines that need to wrap
        start = buffer + pos;
        j = 0;
        ...
        while (j < len) {
            ...
            // Find how many characters to print (handle tab characters)
            while (chars_to_print < window_cols && j < len) {
                if (*end == '\t') {
                    chars_to_print += 8 - (chars_to_print % 8); // Assume 8-character tab stops
                } else {
                    chars_to_print++;
                }
                end++;
...
void handle_keypress(char c) {
    switch (c) {
        case ' ': // Space - move forward one screen
            stop_timer();
            scrolling_active = 0;
        ...
        case 'f': // Increase scrolling speed
        ...
        case 's': // Decrease scrolling speed
            scroll_interval *= (1.0 + SCROLL_RATE_CHANGE);
            if (scrolling_active) {
                set_timer();
            }
...
void display_prompt() {
    ...
    // Create and display the prompt in reverse video
...
// Clean up resources and restore terminal before exiting
void cleanup_and_exit() {
    // Stop the timer
    stop_timer();
